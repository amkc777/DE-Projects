...
def transform(s3_bucket):
    ...
    bucket = s3.Bucket('dend-jose')
    sas_filenames = []

    spark = SparkSession.builder.appName("Transform data").getOrCreate()

    for ix, obj in enumerate(bucket.objects.filter(Prefix='18-83510-I94-Data-2016')):
        if ix == 0:
            df_immigration = spark.read.format('com.github.saurfang.sas.spark').load(s3_bucket + obj.key)
            df_immigration = df_immigration.select('arrdate', 'depdate', 'i94mon', 'i94visa',
                                        'i94port', 'i94addr', 'biryear', 'gender')
        else:
            df_temp = spark.read.format('com.github.saurfang.sas.spark').load(s3_bucket + obj.key)
            df_temp = df_temp.select('arrdate', 'depdate', 'i94mon', 'i94visa',
                                        'i94port', 'i94addr', 'biryear', 'gender')
            df_immigration = df_immigration.union(df_temp)
        sas_filenames.append(s3_bucket + obj.key)
